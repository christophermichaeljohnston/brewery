...
class Fermenter(models.Model):
  ...
  def get_mode(self):
    ...
    self.mode = Device.serial_cmd(self.component.device.device,'getMode')
    self.save()
  ...
  def set_mode(self, mode):
    ...
    if self.mode != mode:
      Device.serial_cmd(self.component.device.device,'setMode,'+mode)
      self.get_mode()
  ...
  def get_setpoint(self):
    from device.models import Device
    self.setpoint = Device.serial_cmd(self.component.device.device,'getSetpoint')
    self.save()
  ...
  def set_setpoint(self, setpoint):
    ...
    if self.setpoint != Decimal(setpoint):
      Device.serial_cmd(self.component.device.device,'setSetpoint,'+setpoint)
      self.get_setpoint()
      if hasattr(self, 'beer'):
  ...
  def get_hysteresis(self):
    from device.models import Device
    self.hysteresis = Device.serial_cmd(self.component.device.device,'getHysteresis')
    self.save()
  ...
  def set_hysteresis(self, hysteresis):
    ...
    if self.hysteresis != Decimal(hysteresis):
      Device.serial_cmd(self.component.device.device,'setHysteresis,'+hysteresis)
      self.get_hysteresis()
      if hasattr(self, 'beer'):
  ...
  def get_anticycle(self):
    from device.models import Device
    self.anticycle = Device.serial_cmd(self.component.device.device,'getAntiCycle')
    self.save()
  ...
  def set_anticycle(self, anticycle):
    ...
    if self.anticycle != Decimal(anticycle):
      Device.serial_cmd(self.component.device.device,'setAntiCycle,'+anticycle)
      self.get_anticycle()
      if hasattr(self, 'beer'):
        Log.objects.create(beer=self.beer, log='set anticycle '+str(anticycle), date=timezone.now())

  def get_antifight(self):
    from device.models import Device
    self.antifight = Device.serial_cmd(self.component.device.device,'getAntiFight')
    self.save()

  def set_antifight(self, antifight):
    from device.models import Device
    from beer.models import Log
    if self.antifight != Decimal(antifight):
      Device.serial_cmd(self.component.device.device,'setAntiFight,'+antifight)
      self.get_antifight()
      if hasattr(self, 'beer'):
        Log.objects.create(beer=self.beer, log='set antifight '+str(antifight), date=timezone.now())
